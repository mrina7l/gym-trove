...
serve(async (req) => {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      console.log('Order details:', { userId, items, total });
      
      if (!userId || !items || !total) {
        throw new Error('Missing order details in session metadata');
      }
      
      // Create the order with a valid status from our enum
      const { data: order, error: orderError } = await supabaseAdmin
        ...
        .insert({
          ...
          total: total,
          status: 'processing', // Use a valid status from the enum directly
